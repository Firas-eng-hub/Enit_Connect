...
noResult = false;
  search(){
    ...
    var reqHeader = new HttpHeaders({ 'Authorization': 'Bearer ' + localStorage.getItem("userToken")});
    this.http.get("http://localhost:3000/"+ this.entity +"/search?property="+this.property+"&key="+this.key, { headers: reqHeader }).subscribe((data : any)=>{
    //this.http.get("http://localhost:3000/student/location?property=firstname&key=dabbabi", { headers: reqHeader }).subscribe((data : any)=>{
      console.log(data);
      this.togglePopupLoading();
  ...
  getAll(){
    ...
    this.http.get("http://localhost:3000/student/" + entite , { headers: reqHeader }).subscribe((data : any)=>{
      console.log(data);
      this.togglePopupLoading();
      if(this.entity == "student"){
        this.users = data;
