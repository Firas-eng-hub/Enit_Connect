import { User, Bell, Lock, Globe, Eye, EyeOff } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const loadPreferences = async () => {
    ...
    try {
      ...
      setPrefsMessage(t('settings.notifications.email') + ' — failed to load.');
    } finally {
  ...
  return (
    <div>
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-2xl px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-10 shadow-xl mb-8">
        ...
        <p className="text-primary-100 text-lg">{t('settings.subtitle')}</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            <div>
              ...
              <input type="email" defaultValue="student@enit.tn" className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all" />
            </div>
            <div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            {[
              { label: t('settings.notifications.email'), desc: t('settings.notifications.emailDesc'), value: emailNotifications, toggle: toggleEmail },
              { label: t('settings.notifications.push'), desc: t('settings.notifications.pushDesc'), value: pushNotifications, toggle: togglePush },
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
          ...
          <div className="p-6">
            <label className="block text-sm font-semibold text-gray-700 mb-2">{t('settings.privacy.visibility')}</label>
            <select value={profileVisibility} onChange={(e) => setProfileVisibility(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all bg-white">
              ...
              <option value="hidden">{t('settings.privacy.hidden')}</option>
            </select>
          </div>
        </div>
